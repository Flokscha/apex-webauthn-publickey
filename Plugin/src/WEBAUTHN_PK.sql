...
CREATE OR REPLACE PACKAGE BODY WEBAUTHN_PK AS
  ...
  PRAGMA EXCEPTION_INIT(USERPRESENT_MISSING, -20014);
  USERVERFICATION_MISSING EXCEPTION;
  PRAGMA EXCEPTION_INIT(USERVERFICATION_MISSING, -20015);
  KEYALG_UNSUP EXCEPTION;
  PRAGMA EXCEPTION_INIT(KEYALG_UNSUP, -20016);
  ID_DUPL EXCEPTION;
  PRAGMA EXCEPTION_INIT(ID_DUPL, -20022);
  ...
  function login_region_ajax ( 
    ...
    return apex_plugin.t_region_ajax_result as 
      ...
      begin 
        ...
        APEX_JSON.CLOSE_ARRAY(); 
        APEX_JSON.WRITE('userVerification', p_region.attribute_04); 
        APEX_JSON.WRITE('timeout', p_region.attribute_05); 
        APEX_JSON.CLOSE_ALL(); 
  ...
  procedure get_challenge(pChallengeItem in Varchar2, pRelyingParty in Varchar2, puserVerification in Varchar2 default 'discouraged') as 
      ...
      userVerification              Varchar2(255) := puserVerification; /*"required","preferred","discouraged"*/
  ...
  procedure register(pChallengeItem in Varchar2, puserVerification in Varchar2 default 'discouraged') as 
    ...
    --   end if; 
      ...
      declare
        clientDataJson Varchar2(4000);
        hashed         Varchar2(4000);
      begin
        clientDataJson := apex_string.format('{"challenge":"%s","crossOrigin":%s,"origin":"%s","type":"%s"}',
          apex_json.GET_VARCHAR2('response.clientDataJSON.challenge'),
          apex_json.GET_VARCHAR2('response.clientDataJSON.crossOrigin'),
          apex_json.GET_VARCHAR2('response.clientDataJSON.origin'),
          apex_json.GET_VARCHAR2('response.clientDataJSON.type')
        );
        hashed := hash_sha256(clientDataJson);
        hash := hashed;
        apex_debug.trace('Hashing ClientDataJSON: %s, hashed: %s', clientDataJson, hashed);
      exception
      when others then
        apex_json.write('error', 'Error in Hashing ClientDataJSON');
        apex_debug.error('Error in Hashing ClientDataJSON: %s, CleintDataJSON: %s', SQLERRM, clientDataJson);
      end;
    ...
    */ 
      ...
      begin
        ...
        if (hash_sha256(OWA_UTIL.GET_CGI_ENV('HTTP_HOST')) != apex_json.get_varchar2('response.authenticatorData.rpIdHashHex') ) then 
          RAISE_APPLICATION_ERROR(-20013, 'Error at verification of authenticatorData.rpID');
        end if; 
      exception
      when RPID_HASHED then
      ...
      declare
        flags          Number;
        userPresentBit boolean;
        userVerified   boolean;
        attestedCredentialData boolean;
        extensionData boolean;
      begin
        apex_debug.trace('JSON response.authenticatorData.flags: %s', apex_json.GET_VARCHAR2('response.authenticatorData.flags'));
        flags := apex_json.GET_VARCHAR2('response.authenticatorData.flags');
        userPresentBit          := BITAND(flags, 1) != 0;
        userVerified            := BITAND(flags, 4) != 0;
        attestedCredentialData  := BITAND(flags, 64) != 0;
        extensionData           := BITAND(flags, 128) != 0;
        apex_debug.trace('Flags: userPresent %s, userVerified %s, AttestedCredential %s, Extensions %s', 
          apex_json.stringify(userPresentBit), 
          apex_json.stringify(userVerified), 
          apex_json.stringify(attestedCredentialData), 
          apex_json.stringify(extensionData)
        );

        if (not userPresentBit) then 
          RAISE_APPLICATION_ERROR(-20014, 'Error at User Presence Flag');
        end if;

      exception
      when USERPRESENT_MISSING then
        apex_json.write('error', 'Error at User Presence Flag'); 
        apex_debug.error('Error at User Presence Flag, %s, Flags: %s', SQLERRM, flags);
        apex_debug.error('Flags: userPresent %s, userVerified %s, AttestedCredential %s, Extensions %s', 
          apex_json.stringify(userPresentBit), 
          apex_json.stringify(userVerified), 
          apex_json.stringify(attestedCredentialData), 
          apex_json.stringify(extensionData)
      ...
      declare
        ...
        userVerified   boolean;
      begin
        apex_debug.trace('User Verification: %s', puserVerification );
        flags := apex_json.GET_VARCHAR2('response.authenticatorData.flags');
        userVerified            := BITAND(flags, 4) != 0;
        if puserVerification in ('required') and not userVerified then
          RAISE_APPLICATION_ERROR(-20015, 'Error in User verification. User Verification is required and is not present!');
        end if;
      exception
      when USERVERFICATION_MISSING then
        apex_json.write('error', 'Error in User verification. User Verification is required and is not present'); 
        apex_debug.error('Error in User verification. User Verification is required and is not present, %s, Flags: %s', SQLERRM, flags);
      end;
      ...
      declare
        key_alg number;
      begin
        key_alg := apex_json.get_number('response.publicKeyAlg');
        apex_debug.trace('JSON Key alg: %s', key_alg );
        if key_alg not in (-7, -257) then --TODO FIXME KEY ALGS
          RAISE_APPLICATION_ERROR(-20016, 'Error in Algorithm verification. Algorithm not supported!');
        end if;
      exception
      when KEYALG_UNSUP then
        apex_json.write('error', 'Error in Algorithm verification. Algorithm not supported'); 
        apex_debug.error('%s, publicKeyAlg: %s', SQLERRM, key_alg);
      end;
    ...
    --   end if; 
      ...
      declare
        found_ids number;
      begin
        select count(*) into found_ids from webauthentication 
         where 
        json_value(responsejson,  '$.id' RETURNING VARCHAR2(4000)) = apex_json.get_varchar2('id');

        apex_debug.trace('found %s credentials (should be 0)', found_ids );
        if found_ids > 0 then
          RAISE_APPLICATION_ERROR(-20022, 'Error in Registration. CredentialId is already in use!');
        end if;
      exception
      when ID_DUPL then
        apex_json.write('error', 'Error in Registration. credentialId is already in use!'); 
        apex_debug.error('%s, found ids: %s', SQLERRM, found_ids);
      end;
      ...
      begin
        ...
        values 
        (v('APP_USER')
        , APEX_UTIL.GET_USER_ID(UPPER(v('APP_USER')))
        , apex_json.get_varchar2('id')
        , apex_json.get_varchar2('response.authenticatorData.signCount')
        , apex_string.join(apex_json.GET_T_VARCHAR2('response.transports'), ':')
        , arg1); 
      end;
    ...
    begin 
      if (apex_application.g_x01 = 'GET_CHALLENGE') then 
        get_challenge(p_dynamic_action.attribute_01, p_dynamic_action.attribute_02, p_dynamic_action.attribute_03); 
      elsif (apex_application.g_x01 = 'REGISTER') then 
        register(p_dynamic_action.attribute_01, p_dynamic_action.attribute_03); 
      end if; 
      return l_plugin; 
